...
class TabContentRPCTab {
    constructor(browserTabId, browserFrameId, tabContentRPC) {
        ...
        this._demandInstanceToken = null;
        Object.seal(this);
    }
    ...
    async call(nameOrOptions, ...params) {
        ...
        if (this._demandInstanceToken) {
            message.rpcInstanceToken = this._demandInstanceToken;
        }

        // (A nice function name which we can easily recognise in stack traces)
        const tabContentRPCTab$call$sendMessage = async () => {
            try {
    ...
    /**
     * Configure this RPC instance to only accept messages annotated with the given `token`.
     * And to annotate all outgoing messages with this `token`.
     * The ContentRPC instance at the other end of the channel must be configured with the same token.
     ...
     * @param {string} token
     */
    demandInstanceToken(token) {
        this._demandInstanceToken = token;
    }
}
...
class TabContentRPC {
    ...
    handleRuntimeMessage(message, messageSender) {
        ...
        if (id !== 'openrunner@computest.nl' || // different extension
            ...
            (tab._demandInstanceToken && message.rpcInstanceToken !== tab._demandInstanceToken) ||
