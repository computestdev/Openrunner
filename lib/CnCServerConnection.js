...
const {WS_CLOSE_TIMEOUT, WS_CLOSE_ABNORMAL_CLOSURE, WS_CLOSE_POLICY_VIOLATION} = require('./webSocketCodes');
...
class CnCServerConnection {
    ...
    _handleWSError(webSocket, err) {
        try {
            ...
            this._markWebSocketAsClosed(webSocket);
            webSocket.close(WS_CLOSE_ABNORMAL_CLOSURE, 'Abnormal Closure: error event');
        }
        catch (err2) {
