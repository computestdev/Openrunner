...
const resolveFullFirefoxPath = async (firefoxPath) => {
    const resolvedPath = path.resolve(firefoxPath);

    if (path.extname(resolvedPath) === '.app') {
        const firefoxStat = await fs.stat(resolvedPath);
        if (firefoxStat.isDirectory()) {
            return path.join(resolvedPath, 'Contents', 'MacOS', 'firefox-bin');
        }
    }
...
const startFirefox = ({fullFirefoxPath, profilePath, headless = false, debugging = false, extraArgs = []}) => Promise.try(async () => {
    log.debug({fullFirefoxPath, profilePath}, 'Starting firefox...');
    ...
    if (debugging) {
        ...
        await debuggingClient.connect();
    }

    return {firefoxProcess, debuggingClient};
})
.disposer(async ({firefoxProcess, debuggingClient}) => {
    log.debug({fullFirefoxPath, profilePath}, 'Stopping firefox...');
    try {
        if (debuggingClient) {
            await debuggingClient.disconnect();
        }
    }
    finally {
        await firefoxProcess.stop();
    }
    log.debug({fullFirefoxPath, profilePath}, 'Stopped firefox');
});

module.exports = {resolveFullFirefoxPath, startFirefox};
