...
class TabManager extends EventEmitter {
    ...
    async frameAncestorIds(tab, browserFrameId) {
        ...
        const result = [];
        let current = browserFrameId;

        for (let it = 0; ; ++it) {
            if (it >= 100) {
                throw Error('TabManager#frameAncestorIds(): Gave up after 100 attempts');
            }
            ...
            const frame = tab.frameByBrowserId(current);
            if (frame) {
